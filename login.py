...
# import libraries
from flask import Flask, redirect, url_for, render_template, request, abort, Blueprint, make_response, jsonify, Response
  ...
  with login_app.app_context():
   if request.method == 'POST':
    try: 
     ...
     signInForm.add_argument('signIn_username', type=str, help='Username is required for logging in', required=True)
     signInForm.add_argument('signIn_password', type=str, help='Password is required for logging in', required=True)
     ...
     create_validated_fields_dict(validated_fields=validated_fields, username=signIn_username, password=signIn_password)
     ...
     if user:
      ...
      #  permissions_list = ['can_view_dashboard', 'can_view_profile', 'can_upload_profile', 'can_update_profile', 'can_use_geolocation_api']
       ...
       for permission in permissions_list:
        grant_permission_response = grant_permission_to_verified_users(permission_name=permission, db=db, Users=Users, Permission=Permission)
       ...
       if grant_permission_response:
        ...
        if user.verification_method == 'Email':
          ...
          response = requests.get(url=aws_sending_otp, headers=headers)
     ...
     # Return error response or redirect to login page wit error message
      ...
      response_json = json.dumps(response_data)
      response = Response(response=response_json, status=404, mimetype='application/json')
      return response
