...
class UserInformation(db.Model):
  ...
  # relationship
  user_id = db.Column(UUID(as_uuid=True), db.ForeignKey('users.user_id'), nullable=False) # form a one-to-one relationship with the user's id in the users table (using foreign key contraint)

  study_preferences = db.relationship('StudyPreferences', backref='user_information', uselist=False, lazy=True)

  availabity_schedule = db.relationship('AvailabilitySchedule', backref='user_information', uselist=False, lazy=True)
...
for i in range(12):
  ...
  list_study_time.append(study_time["Study_time_preferences"][f"{i+1}"]["Time"] + '-' + study_time["Study_time_preferences"][f"{i+1}"]["Frame"])

# append the list of time management methods and study techniques
for i in range(3):
  list_time_management_methods.append(time_management_methods["Time_Management_Methods"][f"Method{i+1}"]["name"])

for i in range(14): 
  list_study_techniques.append(study_techniques["Study_Techniques"][f"technique{i+1}"]["name"])
...
class StudyPreferences(db.Model):
  ...
  time_management_preferences = db.Column(db.Enum(*list_time_management_methods, name='list_time_management_methods'), nullable=False)
...
class AvailabilitySchedule(db.Model):
  ...
  id = db.Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, unique=True, nullable=False)
  user_id = db.Column(UUID(as_uuid=True), db.ForeignKey('user_information.id'), nullable=False)
