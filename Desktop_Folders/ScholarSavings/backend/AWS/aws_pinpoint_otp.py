...
class sendOTP():
 def __init__(self, phoneNumber, appId, allowedAttempts) -> None:
   super().__init__()
 ...
 def verify_otp(self, destinationNumber, otp, brandName, source):
  client = boto3.client('pinpoint', region_name=self.region)
  try:
   ...
   return jsonify({'message' : f'Valid OTP for {self.phoneNumber} : {response}'})
