        ...
        try:
            ...
            for constraint in constraints:
                if constraint["name"] == "age_check":
                    print(
                        f"The 'age_check' constraint already exits on the 'users' table"
                    )
                    break
                else:
                    ...
                    db.engine.execute(
                        "ALTER TABLE users ADD CONSTRAINT age_check CHECK (age = extract(year from age(date_of_birth)))"
                    )
            ...
            print(
                f"There was an error occured while creating all the tables in the database: {e}"
            )
